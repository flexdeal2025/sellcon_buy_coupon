...
export async function POST(req: Request) {
  ...
  if (!fileId) {
    ...
    if (/^(\/(help|start)(@\w+)?|도움말|help|\?)$/i.test(text)) {
      await reply(
        "📖 수집 봇 사용법\n" +
        "먼저 '이미지형' / '코드형' / '증빙' 중 하나를 보내 모드를 고르세요.\n\n" +
        "🖼️ 이미지형 (쿠폰 이미지 재고 등록)\n" +
        "  1) 'YYMMDD 매입처' (예: 260623 당근)\n" +
        "  2) 쿠폰 이미지 업로드 → AI OCR → 검수대기\n\n" +
        "🔢 코드형 (쿠폰 코드 재고 등록)\n" +
        "  1) 'YYMMDD 매입처'\n" +
        "  2) '코드\\n상품명(부분 OK)\\n유효기간(YYMMDD)' 전송\n" +
        "  3) 코드를 줄바꿈으로 전송\n\n" +
        "🧾 증빙 (당근 거래내역 → 재고 매핑)\n" +
        "  1) (선택) 'YYMMDD 매입처' → 매칭 범위 좁힘\n" +
        "  2) 당근 '상세 내역' 캡쳐 업로드\n" +
        "     → 상품명·금액·거래상대·거래번호 자동추출 + 추천 회신\n" +
        "  3) 추천이 맞으면 '확인', 아니면 '스킵'(웹에서 수동 연결)\n\n" +
        "⌨️ 공통 명령\n" +
        "  • 모드 변경: '이미지형'/'코드형'/'증빙' 다시 전송\n" +
        "  • 매입처 변경: 새 'YYMMDD 매입처'\n" +
        "  • 확인: 증빙 추천 연결 / 스킵: 추천 건너뛰기\n" +
        "  • 취소: 직전 등록 묶음 삭제 / 종료(끝): 수집 종료\n" +
        "  • 도움말 다시 보기: /help"
      );
